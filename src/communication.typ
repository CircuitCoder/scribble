...
#common.scribble-post("通讯")[
  ...
  假设 $E(x)$ 里面实际有 $nu <= t$ 个错误 ${ e_j }_(j=1...nu)$，位置分别在 $i_1, ..., i_nu$ 次项，也就是：
  ...
  BM 算法的直觉是一个一个试，看看目前猜的 LFSR 有没有出错。保存一个上一次出错的时候的 LFSR，以及当时出的错 $b$，如果这次出错 $d$，那么把上次的 LFSR $times d / b$ 减掉就可以用来修这次的错。Pseudocode in Rust format:
  ...
  - #link("https://en.wikipedia.org/wiki/Forney_algorithm")[Forney algorithm]: 在已知错误位置后计算错误值。在其中定义的 Error evaluator 是将 RS decoding 建模为 Padé approximation 问题的基础。
  - #link("https://arxiv.org/abs/0906.3778")[Modified Euclidean Algorithms for Decoding Reed-Solomon Codes]
]